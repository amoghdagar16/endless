...
export default function SignUpPage() {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If signup successful and we have a session, create company
      if (signupData.session) {
        try {
          ...
          // Don't fail the signup if company creation fails - user can create it later
          setError("Account created, but company setup failed. Please contact support.");
        }
      } else {
        // Email confirmation required
        setError("");
        alert("Sign up successful! Please check your email to verify your account. After verification, you'll be able to set up your company.");
      }
    } catch (err: any) {
  ...
  const handleGoogleSignUp = async () => {
    try {
      setLoading(true);
      setError("");
      // Google OAuth automatically signs up if user doesn't exist
      // After OAuth, we'll check if they have a company and create one if needed
